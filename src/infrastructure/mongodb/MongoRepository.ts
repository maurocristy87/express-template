...
import { Collection, ObjectId, OptionalId, SortDirection, WithId, WithoutId } from "mongodb";
...
export class MongoRepository implements IMongoRepository {
    ...
    public async persist<T>(document: T): Promise<void> {
        (document as WithId<T>)._id
            ? await this.collection.replaceOne({ _id: (document as WithId<T>)._id }, document as WithoutId<T>)
            : await this.collection.insertOne(document as OptionalId<T>);
    }
